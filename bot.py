...
raw_proxy = os.getenv("HTTP_PROXY", "").strip()
if raw_proxy:
    PROXY_URL = raw_proxy if "://" in (raw_proxy or "") else f"http://{raw_proxy}"
else:
    PROXY_URL = None
...
# Глобальные хранилища данных для УМНОГО ТАЙМЕРА (Сбор безлимитных альбомов)
USER_BATCHES = {}
...
def get_kb():
    return ReplyKeyboardMarkup(
        keyboard=[[KeyboardButton(text="🚀 Рестарт контейнера")]], 
